...
def pdfparsing(list_so, xml_data, headers, cookie_str, i):
    ...
    # 获取so订单web内容
    spec_web = gethtmltext("post", posturl, cookie_str, **headers)
    sospec = getsospec(spec_web)  # 获得成品料号，机型名称，成品规格描述
    print(sospec)
    ...
    Res = gethtmltext("get", so_url, cookie_str, **headers)
    pdf_image(headers['pdfpath'])
...
def main():
    ...
    if len(list_so) != 0:  # 如果列表不为空
        ...
        for i in range(len(list_so)):
            # 处理PDF
            pdfparsing(list_so, xml_data, headers, cookie_str, i)
    else:
        print('无待处理的生产指示单')
